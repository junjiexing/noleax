...
class JsonEmitter {
 ...
 private:
  static void validate_utf8(std::string_view value) {
    ...
    while (index < value.size()) {
      ...
      if (continuation_count > value.size() - index - 1U) {
        throw JsonFormatError{"JSON string ends inside a UTF-8 sequence"};
      }
      const unsigned char second = bytes[index + 1U];
      if (second < second_minimum || second > second_maximum) {
        throw JsonFormatError{"JSON string contains an invalid UTF-8 sequence"};
      }
      for (std::size_t offset = 2U; offset <= continuation_count; ++offset) {
        if (bytes[index + offset] < 0x80U || bytes[index + offset] > 0xbfU) {
          throw JsonFormatError{"JSON string contains an invalid UTF-8 continuation byte"};
        }
      }
