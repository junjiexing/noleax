...
#include "noleax/agent/windows/rtl_reallocate_heap_event.hpp"

namespace noleax::agent::windows {

struct RtlReAllocateHeapHookState;

class RtlReAllocateHeapHook {
 public:
  ...
  explicit RtlReAllocateHeapHook(HookBackend& backend,
                                 std::size_t event_queue_capacity = kDefaultEventQueueCapacity,
                                 std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  RtlReAllocateHeapHook(HookBackend& backend, RtlHeapEventQueue& event_queue,
                        std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  ~RtlReAllocateHeapHook();

  RtlReAllocateHeapHook(const RtlReAllocateHeapHook&) = delete;
  RtlReAllocateHeapHook& operator=(const RtlReAllocateHeapHook&) = delete;
  RtlReAllocateHeapHook(RtlReAllocateHeapHook&&) = delete;
  RtlReAllocateHeapHook& operator=(RtlReAllocateHeapHook&&) = delete;
  ...
  [[nodiscard]] bool try_dequeue_event(RtlReAllocateHeapEvent& event) noexcept;
 ...
 private:
  ...
  std::unique_ptr<RtlReAllocateHeapHookState> hook_state_;
