...
#include "noleax/agent/windows/rtl_destroy_heap_event.hpp"

namespace noleax::agent::windows {

struct RtlDestroyHeapHookState;

class RtlDestroyHeapHook {
 public:
  ...
  explicit RtlDestroyHeapHook(HookBackend& backend,
                              std::size_t event_queue_capacity = kDefaultEventQueueCapacity,
                              std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  RtlDestroyHeapHook(HookBackend& backend, RtlHeapEventQueue& event_queue,
                     std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  ~RtlDestroyHeapHook();

  RtlDestroyHeapHook(const RtlDestroyHeapHook&) = delete;
  RtlDestroyHeapHook& operator=(const RtlDestroyHeapHook&) = delete;
  RtlDestroyHeapHook(RtlDestroyHeapHook&&) = delete;
  RtlDestroyHeapHook& operator=(RtlDestroyHeapHook&&) = delete;
  ...
  [[nodiscard]] bool try_dequeue_event(RtlDestroyHeapEvent& event) noexcept;
 ...
 private:
  ...
  std::unique_ptr<RtlDestroyHeapHookState> hook_state_;
