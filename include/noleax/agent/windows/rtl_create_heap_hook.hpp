...
#include "noleax/agent/windows/rtl_create_heap_event.hpp"

namespace noleax::agent::windows {

struct RtlCreateHeapHookState;

class RtlCreateHeapHook {
 public:
  ...
  explicit RtlCreateHeapHook(HookBackend& backend,
                             std::size_t event_queue_capacity = kDefaultEventQueueCapacity,
                             std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  RtlCreateHeapHook(HookBackend& backend, RtlHeapEventQueue& event_queue,
                    std::uint16_t maximum_stack_depth = kDefaultMaximumStackDepth);
  ~RtlCreateHeapHook();

  RtlCreateHeapHook(const RtlCreateHeapHook&) = delete;
  RtlCreateHeapHook& operator=(const RtlCreateHeapHook&) = delete;
  RtlCreateHeapHook(RtlCreateHeapHook&&) = delete;
  RtlCreateHeapHook& operator=(RtlCreateHeapHook&&) = delete;
  ...
  [[nodiscard]] bool try_dequeue_event(RtlCreateHeapEvent& event) noexcept;
 ...
 private:
  ...
  std::unique_ptr<RtlCreateHeapHookState> hook_state_;
