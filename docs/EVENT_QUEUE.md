...
> 状态：P5.3 Windows x64 create/alloc/realloc/free/destroy 共享队列完成
> 范围：NT Heap 五种原始事件、overflow 归因及后台 writer 消费边界
...
P5.3 组合对象拥有的共享 queue 随对象新建，且当前 adapter 每进程只允许一次成功安装。卸载后才允许
...
底层 queue 保留总 dropped counter；P5.3 的五个 NT Heap hook 还分别维护饱和 dropped counter，使单
consumer 能把 overflow 归因到 api_id。writer 把五个 API 的非零 interval count 汇总为：
...
P5.3 的统一 in-process event 固定为 608 bytes，包含：

- queue sequence；
- QueryPerformanceCounter ticks；
- thread id；
- create/allocate/reallocate/free/destroy operation；
...
默认 adapter 容量为 16,384 个 event。包含 per-slot sequence 后预分配约 9.7 MiB；测试
