...
> 范围：`RtlAllocateHeap` + `RtlFreeHeap` 的进程内 trace path、生命周期配对和安全停止

## 1. 目标与边界

allocator/free replacement 只调用 original、保存固定宽度字段、捕获原始 PC 并尝试入队。压缩、容器
...
只写 `api_id=1`。P5.2 才加入 realloc generation，P5.3 才加入 heap generation，P5.6 前
StackDefinition 仍只保存绝对地址，`module_id` 和 `module_offset` 为零。

## 2. 共享队列与跨 API 顺序

allocate/free 使用同一个预分配 `RtlHeapEventQueue`。统一的 600-byte `RtlHeapEvent` 通过 operation 区分
allocate/free，并包含两种 API 所需的参数、raw result、异常状态和定长栈。成功 reservation 时由队列
分配唯一 sequence，因此不同线程、不同 API 的生命周期顺序不依赖可能倒退或相同的 QPC tick。

两个 hook 分别保存 dropped counter，后台 writer 才能把共享 queue overflow 归因到正确 api_id；底层
queue 的总 dropped counter 只用于队列级诊断。被 queue 拒绝的事件没有 sequence，Loss 因而只保存
数量，不伪造范围。

组合模式的安全调用顺序是：

1. 构造 `HookBackend` 和 `RtlHeapHooks`；后者拥有共享 queue，并让两个 hook 取得各自的固定 TEB
   guard 引用。
2. 在 hook 安装前构造 `RtlHeapTraceWriter`；它校验两个 hook 引用同一个 queue、写 CaptureScope
   metadata，并启动带 `InternalThreadScope` 的 worker。
3. 由 `RtlHeapHooks::install()` 安装 allocate/free，再调用 `begin_capture()`。
4. 目标线程产生事件，worker 按共享 sequence 并发 drain。
5. 调用 `RtlHeapHooks::uninstall()`；两个 hook 都完成 revert、replacement quiescence 和 Hoox flush。
6. 调用 writer `finish()`，完成 final drain、Statistics、EndOfTrace 和 worker join。
7. 最后 shutdown `HookBackend` 并关闭输出流。

组合 writer 会拒绝两个独立 queue、已安装的任一 hook 或未初始化 guard runtime。`begin_capture()` 要求
两个 hook 均已安装；`finish()` 要求两个 hook 均不再 installed/teardown-pending，避免过早写出正常
...
allocate/free 共用一个 stack dictionary，相同栈可以跨 API 复用同一 stack_id。dictionary 容量固定，
...
| queue_full | agent_queue | 两个 hook dropped 的和 | 无；被拒绝事件没有 sequence |
| trace_full | writer | 未落盘事件数 | 首末丢失事件的精确范围 |

Statistics 同时保存 aggregate、`api_id=1` 和组合模式下的 `api_id=2`。每个 API 及 aggregate 都必须
满足：
...
最终 drain 后释放 reserve。组合模式最坏尾部包含两个 56-byte Loss record、一个含两个 API 的
136-byte Statistics record、一个 48-byte EndOfTrace record 及三个 56-byte chunk header，共 464
...
- alloc/free 两组 ApiStatistics、aggregate Statistics、EventStream 数量和 completeness 一致；
- queue/trace dropped 均为零的正常组合路径。

`RtlFreeHeap` 合同、fail-fast、quiescence、CFG/CET 和 Full Page Heap 证据见
