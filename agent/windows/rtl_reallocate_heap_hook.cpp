...
PVOID NTAPI replacement_rtl_reallocate_heap(PVOID heap, ULONG flags, PVOID address,
...
bool RtlReAllocateHeapHook::flush(std::uint32_t max_attempts) noexcept {
  if (state_ == State::kInactive || state_ == State::kRetired) {
    return true;
  }
  if (state_ == State::kInstalled) {
...
bool RtlReAllocateHeapHook::replacement_module_is_pinned() const noexcept {
...
bool RtlReAllocateHeapHook::try_finish_teardown(std::uint32_t max_attempts) noexcept {
