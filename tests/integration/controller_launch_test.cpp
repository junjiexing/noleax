...
int run(int argc, char* argv[]) {
  ...
  const bool marker_created = wait_for_file(marker, 2s);
  if (!session.launched_target() || session.stopped() || !marker_created) {
    std::fprintf(stderr,
                 "launch precondition failed: launched=%u stopped=%u marker=%u exited=%u "
                 "exit=%lu pid=%lu\n",
                 session.launched_target() ? 1U : 0U, session.stopped() ? 1U : 0U,
                 marker_created ? 1U : 0U, session.wait_for_target(0ms) ? 1U : 0U,
                 static_cast<unsigned long>(session.target_exit_code()),
                 static_cast<unsigned long>(session.process_id()));
    return 3;
  }
  ...
  if (!ready_before_main || live.state != noleax::ipc::AgentState::kCapturing ||
    ...
    std::fprintf(stderr,
                 "launch capture failed: ready=%u live=%u final=%u observed=%llu written=%llu "
                 "dropped=%llu\n",
  ...
  std::printf("status=ok kind=launch ready-before-main=1 state=finalized events=%llu\n",
...
int main(int argc, char* argv[]) {
  try {
    ...
    std::fprintf(stderr, "controller launch exception: %s\n", error.what());
    return 20;
  }
}
