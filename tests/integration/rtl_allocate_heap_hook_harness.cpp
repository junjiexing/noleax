...
[[nodiscard]] std::uint32_t verify_event_queue() noexcept {
  ...
  const std::uint64_t total_dropped = allocate_dropped + free_dropped;
  const std::uint64_t total_recordable =
      allocate_hook->recordable_call_count() + free_hook->recordable_call_count();
  if (allocate_hook->event_queue_capacity() != kHarnessQueueCapacity ||
      ...
      allocate_events == 0U || free_events == 0U || captured_stacks == 0U) {
    return 9U;
  }
...
[[nodiscard]] std::uint32_t stop_harness(bool verify_queue = true) noexcept {
  if (hook_set == nullptr || allocate_hook == nullptr || free_hook == nullptr ||
...
NOLEAX_HOOK_HARNESS_EXPORT std::uint32_t noleax_test_rtl_allocate_heap_hook_install() noexcept {
  ...
  try {
    ...
    if (!result.installed()) {
      const auto status = result.allocate.installed()
                              ? static_cast<std::uint32_t>(result.free.status) + 20U
                              : static_cast<std::uint32_t>(result.allocate.status);
