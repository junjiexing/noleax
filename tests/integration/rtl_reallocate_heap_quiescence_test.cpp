...
int main() {
  ...
  noleax::agent::windows::RtlReAllocateHeapHook hook{backend, 1024U, 0U};
  ...
  for (std::size_t worker_index = 0U; worker_index < worker_count; ++worker_index) {
    workers[worker_index] = std::thread{[&, worker_index] {
      ...
      while (!stop.load(std::memory_order_acquire)) {
        ...
        const SIZE_T new_size = static_cast<SIZE_T>(64U + (iteration & 511U));
        void* const original = allocate(heap, 0U, kOriginalSize);
        if (original == nullptr) {
          failures.fetch_add(1U, std::memory_order_relaxed);
          break;
        }
        std::memset(original, static_cast<int>(iteration & 0xffU), kOriginalSize);
        void* const resized = reallocate(heap, 0U, original, new_size);
        ...
        for (std::size_t index = 0U; content_valid && index < kOriginalSize; ++index) {
          content_valid = bytes[index] == static_cast<unsigned char>(iteration & 0xffU);
        }
        if (resized == nullptr || !content_valid || free_heap(heap, 0U, resized) == FALSE) {
          failures.fetch_add(1U, std::memory_order_relaxed);
          if (resized == nullptr) {
            static_cast<void>(free_heap(heap, 0U, original));
          }
          break;
        }
  ...
  std::thread thread_churner{[&] {
    ...
    while (!stop.load(std::memory_order_acquire)) {
      std::thread short_lived{[&] {
        void* const original = allocate(heap, 0U, 48U);
        void* const resized = original == nullptr ? nullptr : reallocate(heap, 0U, original, 96U);
        if (resized == nullptr || free_heap(heap, 0U, resized) == FALSE) {
          failures.fetch_add(1U, std::memory_order_relaxed);
          if (original != nullptr && resized == nullptr) {
            static_cast<void>(free_heap(heap, 0U, original));
          }
        }
      }};
  ...
  if (!all_ready || !reached_pre_uninstall || !reached_post_uninstall ||
    ...
    std::fprintf(stderr,
                 "reallocate quiescence failed: ready=%llu operations=%llu thread_creations=%llu "
                 "failures=%llu uninstall=%u in_flight=%llu calls=%llu/%llu recordable=%llu "
