...
constexpr DWORD kLastErrorSentinel = 0xa1530001U;
...
[[nodiscard]] ExceptionObservation invoke_exceptional_reallocation(
    RtlReAllocateHeapFunction reallocate, PVOID heap, PVOID address) noexcept {
  ExceptionObservation observation;
  SetLastError(kLastErrorSentinel);
  __try {
    observation.result = reallocate(heap, kExceptionFlags, address, kImpossibleSize);
...
int main() {
  ...
  if (heap == nullptr || allocate == nullptr || reallocate == nullptr || free_heap == nullptr) {
    return 2;
  }

  void* const baseline_address = allocate(heap, 0U, 96U);
  if (baseline_address == nullptr) {
    return 3;
  }
  std::memset(baseline_address, 0x5a, 96U);
  const ExceptionObservation baseline =
      invoke_exceptional_reallocation(reallocate, heap, baseline_address);
  const bool baseline_preserved = static_cast<unsigned char*>(baseline_address)[0] == 0x5aU;
  static_cast<void>(free_heap(heap, 0U, baseline_address));
  if (baseline.returned || baseline.exception_status == 0U || !baseline_preserved) {
    std::fprintf(stderr, "baseline HEAP_GENERATE_EXCEPTIONS reallocation did not raise\n");
    return 3;
  }
  ...
  void* const hooked_address = allocate(heap, 0U, 96U);
  if (hooked_address == nullptr) {
    return 4;
  }
  std::memset(hooked_address, 0x5a, 96U);
  ...
  const ExceptionObservation hooked =
      invoke_exceptional_reallocation(reallocate, heap, hooked_address);
  const bool hooked_preserved = static_cast<unsigned char*>(hooked_address)[0] == 0x5aU;

  bool found_exception_event = false;
  noleax::agent::windows::RtlReAllocateHeapEvent event;
  while (hook.try_dequeue_event(event)) {
    if (event.operation == noleax::agent::windows::RtlHeapEventOperation::kReallocate &&
        event.status == noleax::agent::windows::RtlReAllocateHeapEventStatus::kException &&
        ...
        event.address ==
            static_cast<std::uint64_t>(reinterpret_cast<std::uintptr_t>(hooked_address)) &&
        event.result_address == 0U) {
      found_exception_event = true;
    }
  }

  const bool exception_state_valid = same_exception(baseline, hooked) && hooked_preserved &&
                                     hook.recordable_call_count() == recordable_before + 1U &&
                                     hook.failed_call_count() == failed_before + 1U &&
                                     hook.exceptional_call_count() == exceptional_before + 1U &&
                                     hook.replacement_in_flight_count() == 0U &&
  ...
  void* const normal = reallocate(heap, 0U, hooked_address, 160U);
  ...
  if (normal != nullptr) {
    ...
    static_cast<void>(free_heap(heap, 0U, hooked_address));
  }
  ...
  if (!exception_state_valid || !normal_valid ||
    ...
    std::fprintf(stderr,
                 "reallocate SEH contract failed: baseline=0x%08lx hooked=0x%08lx "
                 "last_error=%lu/%lu recordable=%llu failed=%llu exceptional=%llu event=%u "
                 "normal=%u uninstall=%u shutdown=%u\n",
                 ...
                 found_exception_event ? 1U : 0U, normal_valid ? 1U : 0U,
                 static_cast<unsigned int>(uninstall_status), shutdown ? 1U : 0U);
    return 5;
  }

  std::printf("status=ok exception=0x%08lx last_error=%lu event=1 old-valid=1 cleanup=1\n",
              static_cast<unsigned long>(hooked.exception_status),
              static_cast<unsigned long>(hooked.last_error));
  return 0;
}
