...
constexpr DWORD kHeapCorruptionStatus = 0xc0000374U;
...
[[nodiscard]] int run_child(std::string_view test_case, bool hooked) {
  ...
  if (allocate == nullptr || free_heap == nullptr || process_heap == nullptr) {
    return 10;
  }

  noleax::agent::HookBackend backend;
  noleax::agent::windows::RtlFreeHeapHook hook{backend, 64U, 0U};
  ...
  const BOOLEAN unexpected = free_heap(heap, 0U, address);
  return unexpected == FALSE ? 16 : 17;
}
...
int main(int argc, char* argv[]) {
  ...
  constexpr std::array<std::string_view, 3U> cases{"bad-address", "wrong-heap", "double-free"};
  for (const std::string_view test_case : cases) {
    DWORD baseline = 0U;
    DWORD hooked = 0U;
    if (!child_exit_code(executable, test_case, false, baseline) ||
        !child_exit_code(executable, test_case, true, hooked)) {
      std::fprintf(stderr, "cannot run isolated %.*s child\n", static_cast<int>(test_case.size()),
                   test_case.data());
      return 5;
    }
    if (baseline != hooked || baseline != kHeapCorruptionStatus) {
  ...
  std::printf("status=ok cases=3 exit=0x%08lx baseline=hooked\n",
              static_cast<unsigned long>(kHeapCorruptionStatus));
  return 0;
}
