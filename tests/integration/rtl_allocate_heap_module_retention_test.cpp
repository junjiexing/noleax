...
using ModuleReferenced = std::uint32_t (*)() noexcept;
...
int wmain(int argc, wchar_t* argv[]) {
  ...
  const auto module_referenced = reinterpret_cast<ModuleReferenced>(
      GetProcAddress(module, "noleax_test_rtl_allocate_heap_hook_module_referenced"));
  const auto stop =
      reinterpret_cast<Stop>(GetProcAddress(module, "noleax_test_rtl_allocate_heap_hook_stop"));
  if (abi_version == nullptr || install == nullptr || module_referenced == nullptr ||
      stop == nullptr || abi_version() != 1U || install() != 0U) {
    FreeLibrary(module);
    return 4;
  }
  if (module_referenced() != 1U) {
    return 5;
  }
  ...
  // The rendezvous-proven teardown released the adapter's module reference, so the harness DLL
  // is unmapped once the loader drops the caller's own reference.
  ...
  std::printf("status=ok module-released=1\n");
  return 0;
}
