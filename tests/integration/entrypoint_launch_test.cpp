...
int run(int argc, char* argv[]) {
  ...
  if (!ready_before_main || live.state != noleax::ipc::AgentState::kCapturing ||
    ...
    std::fprintf(stderr,
                 "entrypoint launch capture failed: ready=%u live=%u final=%u observed=%llu "
  ...
  std::printf("status=ok kind=entrypoint ready-before-main=1 state=finalized events=%llu\n",
...
int main(int argc, char* argv[]) {
  try {
    ...
    std::fprintf(stderr, "entrypoint launch exception: %s\n", error.what());
    return 20;
  }
}
